...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.financial_information?.length > 0) {
      const create_financial_information = body.financial_information;
      body.financial_information = {
        create: create_financial_information,
      };
    } else {
      delete body.financial_information;
    }
